...
window.log   = function(){}
window.keywordToSite = function(keyword) {
    var scheme = document.body.getAttribute("data-domain-scheme");

    if (scheme) {
        if (window.location.port) {
            return "{0}//{1}:{2}".format(
                window.location.protocol,
                scheme.format(keyword),
                window.location.port
            )
        }
        return "{0}//{1}".format(window.location.protocol, scheme.format(keyword));
    }
